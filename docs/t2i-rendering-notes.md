...
默认主卡遵循“内容宽度 + 两侧受控留白 = viewport 宽度”的精确画布原则：`780 + 16 × 2 = 812`。这不会完全移除卡片周围的留白，而是只保留圆角、阴影所需的设计空间，避免全局 T2I 默认宽视口产生非预期的大块白边。
...
专用卡片如果使用不同于默认通知模板的宽度，调用 `trim_viewport_whitespace()` 时必须同时传入自己的 `card_width`、`body_padding` 和 `canvas_width`。例如帮助卡片使用 `868px` 内容宽度和 `900px` 画布；若仍沿用默认 `780px` 推断值，后处理会错误裁掉卡片右侧。卡片高度继续交给 `full_page=true` 与内容流自动决定，不设置静态 clip。

复杂流程的独立甘特图使用同一原则，但不采用固定 `812px` 画布。Renderer 根据任务数量、名称折行、时间跨度和显示密度生成共享 layout metadata；Server 使用其中的 `viewport_width`、`viewport_height`、`card_width` 和 `body_padding` 完成完整页面截图与后处理裁边。截图参数和裁剪参数必须来自同一份 layout，禁止分别重复估算，否则容易出现右侧状态列或最后一行被裁切。

因此当前完整策略可概括为：CSS 收缩页面容器、显式设置精确 viewport、获取本地截图、校验图片格式，再按已知几何与底部像素差异裁掉视口冗余。全局 T2I 负责浏览器截图，插件负责最终画布边界和视觉留白。
...
适配到本插件时，默认主卡、自定义模板和独立甘特图可以使用不同画布宽度。每条渲染路径都必须携带自己的画布几何参数；只有默认主卡可以使用 `780px` 内容宽度与 `812px` viewport 的默认值。
