...
注意：endpoint 记录中的 `render_mode` 和 `template` 字段已在 MVP 移除。
旧 registry JSON 中的残留值被自然忽略，重新保存后不再保留。
渲染模式改为读取插件全局配置。自定义模板功能将在后续版本另行设计。
...
- 自定义模板文件加载（`template`）在后续版本中另行设计。当前只使用默认 HTML 模板。
...
当前 HTML renderer 使用 Jinja2 `SandboxedEnvironment` 但将 `autoescape` 设为 `False`。理由如下：

- 渲染输出用于服务端 T2I 截图，不直接展示在浏览器中。
- 模板来源受管理员控制，不允许普通聊天用户提交。
- 默认模板已在模板层面过滤 `token`、`raw`、`prompt` 等敏感字段，不依赖 autoescape 做唯一防护。
- 后续如果引入用户可控模板变量（例如自定义字段值中包含 HTML 标签），应评估启用 `autoescape` 并适配模板中已使用的 `|safe` 过滤器。
