...
def test_stable_versions_are_pep440_equivalent() -> None:
    ...
    assert package_plugin.versions_equivalent("v1.3.0", "1.3.0")
    assert package_plugin.project_version_for_package("v1.3.0") == "1.3.0"
...
def test_default_release_archive_uses_semver_tag_name() -> None:
    ...
    assert (
        ...
        == "astrbot_plugin_webhook_notifier-v1.3.0.zip"
    )
...
def test_dev_version_with_label_builds_end_to_end(tmp_path: Path, monkeypatch) -> None:
    ...
    plugin_version = "v1.3.0-test.20260720.0905.template-manager"
    assert metadata["version"] == plugin_version
    assert re.search(rf'"{re.escape(plugin_version)}"', main_text)
    project_version = project["project"]["version"]
    assert project_version == "1.3.0.dev202607200905+template.manager"
